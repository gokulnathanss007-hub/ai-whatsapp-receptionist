...
**voice call** (V3), and the **school dashboard** (V4).
...
- **Short blocks:** 2–3 sentences; structured summaries use hyphen lists (the enquiry
...
- **Real slot lists** are system-rendered (never model-typed) and numbered/labelled
  exactly as generated ("Today – 4:30 PM"), so a parent can answer "the 4:30 one".
- **Location/maps** shared as the school's `maps_url` from knowledge.
...
|---|---|---|
| Yes/no or ≤3 fixed choices | **Reply buttons** (max 3) | "Book a school visit?" [Confirm] [Pick another time] |
| 4–10 options | **List message** | Main Menu; available slots; programs/grades menu |
| "Where are you?" | **Location message** | Pin + address card |
| Fee sheets, admission brochures | **Media/PDF** | `send_pdf` action |
...
R: [text] "Of course — I can help you book a visit."
   [list] "Available times" → 6 rows of real slots
P: (taps "Tomorrow – 10:00 AM")            ← arrives as interactive.list_reply
R: [buttons] "Book tomorrow 10:00 AM for your admission visit?" [Confirm] [Pick another time]
P: (taps Confirm)
...
## 3. School dashboard — V4 (design intent)
...
- **Home:** today's bookings, handoff queue (badge count), enquiries this week,
  conversion tile ("X enquiries → Y visits booked").
- **Conversations:** searchable timeline per parent (all channels merged).
...
- **Knowledge editor:** edit FAQs/services (grades)/hours/fees as forms → bumps
...
- **Multi-school switcher** (organizations) for group owners.

Prerequisite: real authentication/authorization system — the `ADMIN_SETUP_TOKEN`
stopgap never carries user-facing UI.
...
1. Same brain everywhere — knowledge, intents, safety, booking are shared; surfaces only
   render.
2. The parent-facing voice (tone, brevity, warmth) is identical in text, buttons, and
   speech — `PRODUCT.md` §7 binds all surfaces.
3. Every surface writes to the same conversation/parent timeline.
