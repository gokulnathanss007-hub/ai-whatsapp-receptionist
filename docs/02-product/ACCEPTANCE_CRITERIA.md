...
6. **No school data in code**; no secrets outside env; no PII or tokens in logs.
7. **Migrations append-only**, shipped with the indexes their queries need.

## 2. V1 acceptance criteria (met — preserved record)

**AI WhatsApp Parent Enquiry System**
- A real parent thread runs end-to-end: greeting → FAQ → qualification → admission
  enquiry recorded → correct handoff on a sensitive/complaint message.
- Replies on-spec (short, warm, never promising admission/inventing facts) on a
  staff-reviewed sample.
...
- Escalation triggers (sensitive_matter/complaint/billing_issue/refund/
  urgent_safety_concern/legal/unknown) all produce handoff + reason code, both via model
  output AND independent code detection.
...
- Model echoes real slot ids only; contradictory outputs (both `enquiry_request`
...
- Missed-call webhook → WhatsApp outreach in <60s; thread continues through the normal
  front-office flow; duplicate webhooks deduped on provider call id.
...
**Tamil/Tanglish**
- Per-school locale switches knowledge + prompt language; safety behaviours verified
...
- Safety parity: the full escalation matrix (urgent_safety_concern/sensitive_matter/
  complaint/…) verified by voice test calls; urgent safety concern calls urge an
  immediate direct call to the school office and warm-transfer.
- Every call produces a timeline entry + WhatsApp follow-up on the same parent record.

## 5. V4 acceptance criteria (bar to ship)

- Dashboard actions are authorized per real user/role (no shared tokens); RLS policies
  enforce school isolation for user-facing access.
...
- Multi-school rollups never leak a school's data to a non-member (tested per role).

## 6. Safety acceptance criteria (every version, never waived)

- Zero paths where the model's unreviewed text reaches a parent after a booking
  conflict, parse failure, or safety trigger.
- Safety-intent precedence (`urgent_safety_concern` > `sensitive_matter` >
  complaint/billing_issue/refund > rest) holds in mixed-intent messages.
- `mobile` in any record is always `parent.wa_phone`, never model-supplied.
